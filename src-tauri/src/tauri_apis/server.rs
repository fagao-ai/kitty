...
/// Start a specific xray server by ID.
#[tauri::command(rename_all = "snake_case")]
pub async fn start_xray_server_by_id<'a>(
    state: State<'a, DatabaseState>,
    process_manager: State<'a, ProcessManagerState>,
    xray_id: i32,
) -> CommandResult<KittyResponse<()>> {
    ...
    let mut server_manager = ServerManager::new();
    server_manager.start_xray_server(&db, xray_id).await?;

    // Store the running servers
    let mut servers = process_manager.running_servers.lock().await;
    servers.extend(server_manager.running_servers);

    Ok(KittyResponse::default())
}

/// Start a specific hysteria server by ID.
#[tauri::command(rename_all = "snake_case")]
pub async fn start_hysteria_server_by_id<'a>(
    state: State<'a, DatabaseState>,
    process_manager: State<'a, ProcessManagerState>,
    hysteria_id: i32,
) -> CommandResult<KittyResponse<()>> {
    ...
    let mut server_manager = ServerManager::new();
    server_manager.start_hysteria_server(&db, hysteria_id).await?;
